...
@interface YYYEditGroupAlbumViewController ()

@end

@implementation YYYEditGroupAlbumViewController
...
-(void)showPhotos
{
	for (int i = 0; i < [lstPhoto count]; i++)
	{
		...
		btPhoto.tag = 100 + [[[lstPhoto objectAtIndex:i] objectForKey:@"id"] intValue];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
	...
	void ( ^successed )( id _responseObject ) = ^( id _responseObject )
	{
		...
		{
			lstPhoto = [[NSMutableArray alloc] initWithArray:[_responseObject objectForKey:@"photoarray"]];
			[self showPhotos];
		}
		else
		{
	...
	[[YYYCommunication sharedManager] UploadGroupPhoto:[[YYYCommunication sharedManager].dictInfo objectForKey:@"user_id"]
											   groupid:groupID
...
{
	[delegate DidAlbumUpdated:lstPhoto];
