...
@interface YYYMessageViewController ()

@end

@implementation YYYMessageViewController
...
- (void)viewDidLoad
{
	...
	[self.navigationItem setTitle:[dictUser objectForKey:@"user_name"]];
...
-(void)loadIncoming:(NSNotification*)notif
{
	...
	for (NSDictionary *dict in  [notif.userInfo objectForKey:@"income"])
	{
		if([[dict objectForKey:@"useridfrom"] intValue] != [[dictUser objectForKey:@"user_id"] intValue])
			continue;
		...
		if (nLastMsgIndex < [[dict objectForKey:@"id"] intValue])
		{
            ...
            NSString *avatarURL = [dictUser objectForKey:@"user_avatar"];
            NSString *facebookID = [dictUser objectForKey:@"user_facebookid"];
            ...
            {
                avatarURL = [NSString stringWithFormat:@"%@/%@",WEBAPI_URL,[dictUser objectForKey:@"user_avatar"]];
            }
			...
			if ([message rangeOfString:PHOTOBOUND].location == 0)
			{
				...
				sayBubble.avatar_url = [NSURL URLWithString:avatarURL];
	...
	if (nMsgCount)
	{
		...
		[[YYYCommunication sharedManager] ReadMessage:[[YYYCommunication sharedManager].dictInfo objectForKey:@"user_id"] useridto:[dictUser objectForKey:@"user_id"] successed:nil failure:nil];
	}
}
...
-(void)loadMessageHistory
{
	void ( ^successed )( id _responseObject ) = ^( id _responseObject )
	{
		...
		{
			if ([[_responseObject objectForKey:@"block"] isEqualToString:@"0"])
			{
				[vwBottom setHidden:NO];
				[vwBlock2 setHidden:YES];
				[vwBlock1 setHidden:YES];
                
                if ([[_responseObject objectForKey:@"blocking"] isEqualToString:@"1"])
                {
                    [vwBlock2 setHidden:YES];
                    [vwBottom setHidden:YES];
                    [vwBlock1 setHidden:NO];
                }
			}
			else if ([[_responseObject objectForKey:@"block"] isEqualToString:@"1"])
			{
			...
			for (NSDictionary *dict in [_responseObject objectForKey:@"detail"])
			{
				...
				if ([[dict objectForKey:@"useridfrom"] intValue] == [[dictUser objectForKey:@"user_id"] intValue])
				{
                    ...
                    avatarURL = [dictUser objectForKey:@"user_avatar"];
                    facebookID = [dictUser objectForKey:@"user_facebookid"];
...
//                }
	...
	[[YYYCommunication sharedManager] LoadMessageHistory:[[YYYCommunication sharedManager].dictInfo objectForKey:@"user_id"] useridto:[dictUser objectForKey:@"user_id"] successed:successed failure:failure];
}
...
-(void)sendMessage :(NSString*)text Photo:(NSData*)photo Video:(NSData*)video Voice:(NSData*)voice Location:(NSString*)location Sticker:(NSString*)sticker
{
    ...
    void ( ^successed )( id _responseObject ) = ^( id _responseObject )
	{
		...
		if ([[_responseObject objectForKey:@"success"] isEqualToString:@"1"])
		{
            ...
            [[YYYAppDelegate sharedDelegate] sendPushNotification:[dictUser objectForKey:@"user_id"] :[NSString stringWithFormat:@"%@:%@",[[YYYCommunication sharedManager].dictInfo objectForKey:@"user_username"],pushContent]];
	...
	if (text)
	{
		...
		[[YYYCommunication sharedManager] SendMessage:[[YYYCommunication sharedManager].dictInfo objectForKey:@"user_id"]
											 useridto:[dictUser objectForKey:@"user_id"]
											  message:text
											successed:successed
											  failure:failure];
	}
	...
	{
		[[YYYCommunication sharedManager] SendFile:[[YYYCommunication sharedManager].dictInfo objectForKey:@"user_id"]
										  useridto:[dictUser objectForKey:@"user_id"]
											  type:@"photo"
											  file:photo
										 successed:successed
